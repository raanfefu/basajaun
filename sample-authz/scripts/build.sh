...
GOOS=`cat  ../charts/sample/values.yaml | yq | jq -r .json_values.build_values.golang.goos`
GOARCH=`cat  ../charts/sample/values.yaml | yq | jq -r .json_values.build_values.golang.goarch`
APP_NAME=`cat  ../charts/sample/values.yaml | yq | jq -r .json_values.deployment.name`
...
if [ $? -eq 0 ]; then
    ...
    cd ${BUILD_PATH} && docker build --platform=${DOCKER_PLATFORM} -t ${REGISTRY}/${ADMISSION_CRTL_IMAGE} -f Dockerfile.sample .
else
    exit -1
fi
